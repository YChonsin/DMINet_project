# Code Implementation of the DMI-Net Model
import math
import torch
...
class ChannelAttention(nn.Module):
    """Channel attention originated in RCAN.
    Args:
    ...
    def __init__(self, embed_dim, squeeze_factor=16):
        ...
        self.attention = nn.Sequential(
            ...
            nn.Conv2d(embed_dim, embed_dim // squeeze_factor, 1, padding=0),
            nn.ReLU(inplace=True),
            nn.Conv2d(embed_dim // squeeze_factor, embed_dim, 1, padding=0),
            nn.Sigmoid())
...
class CAB(nn.Module):
    def __init__(self, embed_dim, compress_ratio=3,squeeze_factor=30):
        super(CAB, self).__init__()
        self.cab = nn.Sequential(
            nn.Conv2d(embed_dim, embed_dim // compress_ratio, 3, 1, 1),
            nn.GELU(),
            nn.Conv2d(embed_dim // compress_ratio, embed_dim, 3, 1, 1),
            ChannelAttention(embed_dim, squeeze_factor)
        )
...
class VMBlock(nn.Module):
    def __init__(
        ...
        # print(self.d_model, self.d_inner)
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        self.self_attention = VMBlock(d_model=hidden_dim, d_state=d_state, expand=expand, dropout=attn_drop_rate, **kwargs)
